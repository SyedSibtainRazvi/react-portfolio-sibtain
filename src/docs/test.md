...
Open the web browser and go to your Google Developer Console. Once you open the console, sign in with the Google account to proceed.

Click on the "Select a project" dropdown menu at the top of the screen after login in. Next, click on "New Project".

In the "New Project" dialog, enter a name for the project. Select a name that accuratly reflects the purpose of the application. If the project is being worked on within an organization, choose that organization so that access is limit to individuals who have organization emails. Choosing organization is optional.

Finally, click on "Create" button to create the new project.

Once the project is set up, we need to setup our OAuth consent screen in the API and services section. Click on the "Credentials" >> "Create Credentials" >> "OAuth client ID".

We will be required to configure the OAuth consent page in the following step before we can enable the API. We can simply follow the on-screen instructions to configure the OAuth consent screen with the required informations.

We can use the "internal" or "external" setting to determine which emails can access the login on Google. "Internal" restricts access to users within our organizations, while "external" allows any user with a Google Account to access.

In the authorized redirect URLs, set it to "https://localhost:300/api/auth/callback/google" for local developments or our web app's hosted URL for the production environment.

Once this is done, click save, and then we will get the client ID and client secret for our app.
...
Install require dependencies for authentication and database connectivities
...
Now that we've configured Google Sign-In in our Next.js application, let's run the application and see the authentication flow in actions.

Run the following command to start the development server:

npm run dev

Once the server is running, open the web browser and navigate to http://localhost:3000. Since we've protected the entire layout with authentications, we'll be redirected to the sign-in page automatically.
...
After successful authentications, we'll be redirected back to the application, where we'll have access to the protected content.
